...
Node* init(int a) // Принимаемые данные для головы списка
{
	Node *lst;
...
void freeStruct(Node ** head)/*Освобождение памяти текущего узла*/
{
	Node * current = *head;
	Node * next = NULL;
	do
...
void inputData(Node** headRef, int *countItemsRet, int *itemNumRet)
{
	...
	char line[MAXLEN];
	Node* item_ptr = NULL;
	fpin = fopen("base.txt", "r"); // открыть файл для чтения
	if (fpin == NULL)
	{
...
void recordData(Node** headRef, int itemNum)
{
	FILE *fpin;
	Node* current = *headRef;
	fpin = fopen("base.txt", "w"); // открыть файл для чтения
	fprintf(fpin, "%d\n", itemNum);
	while (current != NULL)
	{
		fprintf(fpin, "(\n");
