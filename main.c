...
void alphSort(Node** headRef, int structType, int count)
{
    Node* current;
    Node* next;
    int noSwap;
...
void getData(Node** headRef, int *countItemsRet, int *itemNumRet)
{
    FILE *fpin;
    FILE *finit;
    int dataNum;
...
void putData(Node** headRef, int itemNum)
{
    FILE *fpin;
    Node* current = *headRef;
...
int main()
{
    ...
    getData(&HeadPointer, &countItems, &itemNum);
    ...
    do
    {
        ...
        switch (need_menu)
        {
            ...
            case 4:
                ...
                alphSort(&HeadPointer, need_showCategory, itemNum);
                ItemPointer = HeadPointer;
                i = 1;
                while (ItemPointer != NULL)
                {
    ...
    if (itemNum)
    {
        putData(&HeadPointer, countItems);
    }
    freeList(&HeadPointer);
    return 0;
}
